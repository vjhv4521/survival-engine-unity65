...
namespace SurvivalEngine.EditorTool
{
    ...
    public class DuplicateObject : ScriptableWizard
    {
        ...
        private readonly Dictionary<GameObject, string> copied_prefabs = new Dictionary<GameObject, string>();
...
;
        ...
        private GameObject CopyPrefab(GameObject prefab, string title)
        {
            if (prefab != null)
            {
                ...
                if (copied_prefabs.ContainsKey(prefab))
                {
                    string cpath = copied_prefabs[prefab];

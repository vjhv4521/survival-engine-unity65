...
namespace SurvivalEngine
{
    ...
    public class JoystickMobile : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (controls.IsLeftClick() && controls.IsInGameplay())
            {
                if (!fixed_position)
                    joystick_pos = controls.GetMousePosition();
                joystick_dir = Vector2.zero;
                joystick_active = false;
                float nrange = range * Screen.height;
                float diff = Vector3.Distance(joystick_pos, controls.GetMousePosition());
                joystick_down = diff < nrange;
            }

            if (!controls.IsLeftHold())
            {
                joystick_active = false;
                joystick_down = false;
                joystick_dir = Vector2.zero;
            ...
            if (controls.IsLeftHold() && joystick_down)
            {
                Vector2 mpos = new Vector2(controls.GetMousePosition().x, controls.GetMousePosition().y);

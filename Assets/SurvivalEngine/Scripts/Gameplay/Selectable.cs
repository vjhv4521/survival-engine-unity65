...
namespace SurvivalEngine
{
    ...
    public class Selectable : MonoBehaviour
    {
        ...
        [Header("Optimization")]
        public bool static_object = false; //Set to true for objects that never move, for optimization
        ...
        private static LinkedList<Selectable> static_list = new LinkedList<Selectable>();
        private static LinkedList<Selectable> dynamic_list = new LinkedList<Selectable>();
        private LinkedListNode<Selectable> node;

        void Awake()
        {
            if (static_object)
                node = static_list.AddLast(this);
            else
                node = dynamic_list.AddLast(this);

            active_node = active_list.AddLast(this);
        ...
        {
            if (static_object)
                static_list.Remove(node);
            else
                dynamic_list.Remove(node);
        ...
        public void SetActive(bool visible)
        {
            if (is_active != visible)
            {
                ...
                {
                    bool turn_off_gameobject = GameData.Get().optim_turn_off_gameobjects;
                    ...
                    else if (active_node.List == active_list)
                        active_list.Remove(active_node);
                }
            }
        }
        ...
        {
            if (static_object && Optimizer.Exists())
            {
                OptimizationZone zone = Optimizer.Get().GetZone(transform.position, true);
                if(zone != null)
                    zone.AddObject(this);
            }
        }
        ...
        public void RemoveFromZones()
        {
            if (static_object && Optimizer.Exists())
            {
                OptimizationZone zone = Optimizer.Get().GetZone(transform.position);
                if (zone != null)
        ...
        //Get ALL static selectables (Careful, this list can be very big for large maps, so dont loop on it each frame)
        public static LinkedList<Selectable> GetAllStatic()
        {
            return static_list;
        }

        //Get ALL dynamic selectables (Careful, this list can be very big for large maps, so dont loop on it each frame)
        public static LinkedList<Selectable> GetAllDynamic()
        {
            return dynamic_list;
        }
    }
